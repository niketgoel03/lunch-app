...
(async () => {
  ...
  console.log('STAFF_ADMIN delegated operations');
  const staff1U = (await call(admin, 'GET', '/api/users')).data.find(u => u.email === 'staff1@office.local');
  await call(admin, 'PUT', '/api/users/' + staff2U.id, { role: 'staff_admin' });
  ok((await call(staff2, 'GET', '/api/me')).data.user.role === 'staff_admin', 'admin promoted user to staff_admin');
  ok((await call(staff1, 'GET', '/api/admin/tasks')).status === 403, 'regular staff cannot manage tasks');
  ok((await call(staff2, 'GET', '/api/people')).status === 200, 'staff_admin can list people');
  const t3 = await call(staff2, 'POST', '/api/admin/tasks', { title: 'Pickup parcel', assignee_id: rahulU.id });
  ok(t3.status === 200, 'staff_admin assigns a task to Rahul (office boy)');
  ok((await call(null, 'GET', '/api/boy/tasks', null, H(key2))).data.some(t => t.id === t3.data.id), 'assigned task shows in office-boy task list');
  const ob = await call(staff2, 'POST', '/api/orders/for/' + staff1U.id, { items: [{ menu_item_id: m.data.id, qty: 1 }] });
  ok(ob.status === 200 && ob.data.order.payment.amount === 120, 'staff_admin places an order on behalf of staff (120)');
  const cat2 = await call(staff2, 'POST', '/api/admin/task-categories', { name: 'Errands' });
  ok((await call(staff2, 'PUT', '/api/admin/task-visibility', { map: { [cat2.data.id]: [staff1U.id] } })).status === 200, 'staff_admin bulk-saves visibility (single action)');
  ...
  const notif = await call(admin, 'POST', '/api/admin/notify', { title: 'Test broadcast', body: 'hello', target: 'all' });
  ok(notif.status === 200 && notif.data.recipients > 0, 'admin broadcast to everyone');
